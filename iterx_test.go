...
func (n FullName) MarshalCQL(info gocqlv2.TypeInfo) ([]byte, error) {
	return []byte(n.FirstName + " " + n.LastName), nil
}

func (n *FullName) UnmarshalCQL(info gocqlv2.TypeInfo, data []byte) error {
...
func diff(t *testing.T, expected, got any) {
	t.Helper()
...
func TestIterxUDT(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TYPE cqlx_test.UDTTest_Full (first text, second text)`); err != nil {
		t.Fatal("create type:", err)
	}

	if err := session.ExecStmt(`CREATE TABLE cqlx_test.udt_table (
			testuuid       timeuuid PRIMARY KEY,
			testudt        cqlx_test.UDTTest_Full
		)`); err != nil {
	...
	makeStruct := func(testuuid gocqlv2.UUID, insert any) any {
		b := reflect.New(reflect.StructOf([]reflect.StructField{
			{
				Name: "TestUUID",
				Type: reflect.TypeOf(gocqlv2.UUID{}),
			},
			{
				Name: "TestUDT",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Run("insert-bind-struct-map", func(t *testing.T) {
				...
				t.Run("empty-struct", func(t *testing.T) {
					if err := session.Query(insertStmt, []string{"test_uuid", "test_udt"}).
						BindStructMap(struct{}{}, map[string]any{
							"test_uuid": testuuid,
							"test_udt":  tc.insert,
						}).ExecRelease(); err != nil {
						t.Fatal(err.Error())
...
func TestIterxStruct(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TYPE cqlx_test.FullName (first_Name text, last_name text)`); err != nil {
		t.Fatal("create type:", err)
	}

	if err := session.ExecStmt(`CREATE TABLE cqlx_test.struct_table (
			testuuid       timeuuid PRIMARY KEY,
			...
			testinet       inet,
			testcustom     text,
			testudt        cqlx_test.FullName,
			testptrudt     cqlx_test.FullName
		)`); err != nil {
		t.Fatal("create table:", err)
	}

	type StructTable struct {
		Testuuid      gocqlv2.UUID
		Testvarchar   string
		Testbigint    int64
...
func TestIterxScannable(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.scannable_table (testfullname text PRIMARY KEY)`); err != nil {
		t.Fatal("create table:", err)
	}
...
func TestIterxStructOnly(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.struct_only_table (first_name text, last_name text, PRIMARY KEY (first_name, last_name))`); err != nil {
		t.Fatal("create table:", err)
	}
...
func TestIterxStructOnlyUDT(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.struct_only_udt_table (first_name text, last_name text, PRIMARY KEY (first_name, last_name))`); err != nil {
		t.Fatal("create table:", err)
	}
...
func TestIterxStrict(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.strict_table (testtext text PRIMARY KEY, testtextunbound text)`); err != nil {
		t.Fatal("create table:", err)
	}
	...
	const (
		...
		golden = "missing destination name \"testtextunbound\" in cqlx_test.StrictTable"
	)
...
func TestIterxNotFound(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.not_found_table (testtext text PRIMARY KEY)`); err != nil {
		t.Fatal("create table:", err)
	}
	...
	t.Run("get", func(t *testing.T) {
		...
		if err != gocqlv2.ErrNotFound {
			t.Fatalf("Get() error=%q expected %s", err, gocqlv2.ErrNotFound)
		}
	})
	...
	t.Run("select", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Select() error=%q expected %s", err, gocqlv2.ErrNotFound)
		}
		if cap(v) > 0 {
			t.Fatalf("Select() effect alloc cap=%d expected 0", cap(v))
		}
...
func TestIterxErrorOnNil(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.nil_table (testtext text PRIMARY KEY)`); err != nil {
		t.Fatal("create table:", err)
	}
...
func TestIterxPaging(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.paging_table (id int PRIMARY KEY, val int)`); err != nil {
		t.Fatal("create table:", err)
	}
	if err := session.ExecStmt(`CREATE INDEX id_val_index ON cqlx_test.paging_table (val)`); err != nil {
		t.Fatal("create index:", err)
	}

	q := session.Query(qb.Insert("cqlx_test.paging_table").Columns("id", "val").ToCql())
	for i := range 5000 {
	...
	stmt, names := qb.Select("cqlx_test.paging_table").
		Where(qb.Lt("val")).
		AllowFiltering().
...
func TestIterxCAS(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.cas_table (id int PRIMARY KEY, salary int)`); err != nil {
		t.Fatal("create table:", err)
	}
	...
	// Apache Cassandra returns only the [applied] column when an LWT succeeds,
	// so GetCAS leaves dest untouched on applied=true. Scylla also returns a
	// pre-image in this case; cqlx targets Apache, hence the asymmetry below.
	...
	if john.Salary != baseSalary {
		t.Errorf("GetCAS()=%v expected dest untouched on applied=true", john)
	}

	// salary in DB is now minSalary; the same IF (salary < minSalary) is false,
	// so the LWT is rejected and Cassandra returns the current value of the
	// condition column — that's the pre-image expected in dest.
	john.Salary = 0
	applied, err = update.BindStructMap(john, qb.M{"min_salary": minSalary}).GetCAS(&john)
	if err != nil {
		t.Fatal("GetCAS() failed:", err)
	...
	if applied {
		t.Error("GetCAS() expected update not to be applied")
	}
	if john.Salary != minSalary {
		t.Errorf("GetCAS()=%v expected pre-image salary=%d on applied=false", john, minSalary)
	}
}
