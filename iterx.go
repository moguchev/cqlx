...
// that also implements gocqlv2.UDTUnmarshaler or in rare cases gocqlv2.Unmarshaler.
...
func (iter *Iterx) scanAny(dest any) bool {
...
func (iter *Iterx) scanAll(dest any) bool {
	value := reflect.ValueOf(dest)
...
//   - ptr to t implements gocqlv2.Unmarshaler, gocqlv2.UDTUnmarshaler or UDT
//   - it is not a struct
...
// StructScan is like gocqlv2.Iter.Scan, but scans a single row into a single
...
func (iter *Iterx) structScan(value reflect.Value) bool {
	...
	if iter.fields == nil {
		...
		iter.values = make([]any, len(columns))
		if cas {
...
func (iter *Iterx) fieldsByTraversal(value reflect.Value, traversals [][]int, values []any) error {
...
func columnNames(ci []gocqlv2.ColumnInfo) []string {
