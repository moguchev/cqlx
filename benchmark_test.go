...
CREATE TABLE IF NOT EXISTS cqlx_test.bench_person (
    id int,
    first_name text,
    last_name text,
    email list<text>,
...
func BenchmarkBaseGocqlInsert(b *testing.B) {
	people := loadFixtures()
	session := cqlxtest.CreateSession(b)
	defer session.Close()
	...
	stmt, _ := qb.Insert("cqlx_test.bench_person").Columns(benchPersonCols...).ToCql()
...
func BenchmarkGocqlxInsert(b *testing.B) {
	...
	stmt, names := qb.Insert("cqlx_test.bench_person").Columns(benchPersonCols...).ToCql()
...
func BenchmarkBaseGocqlGet(b *testing.B) {
	...
	stmt, _ := qb.Select("cqlx_test.bench_person").Columns(benchPersonCols...).Where(qb.Eq("id")).Limit(1).ToCql()
	q := session.Session.Query(stmt)
...
func BenchmarkGocqlxGet(b *testing.B) {
	...
	stmt, names := qb.Select("cqlx_test.bench_person").Columns(benchPersonCols...).Where(qb.Eq("id")).Limit(1).ToCql()
	q := session.Query(stmt, names)
...
func BenchmarkBaseGocqlSelect(b *testing.B) {
	...
	stmt, _ := qb.Select("cqlx_test.bench_person").Columns(benchPersonCols...).Limit(100).ToCql()
	q := session.Session.Query(stmt)
...
func BenchmarkGocqlxSelect(b *testing.B) {
	...
	stmt, names := qb.Select("cqlx_test.bench_person").Columns(benchPersonCols...).Limit(100).ToCql()
	q := session.Query(stmt, names)
...
func initTable(b *testing.B, session cqlx.Session, people []*benchPerson) {
	b.Helper()
