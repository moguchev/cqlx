...
func TestQueryxBindStruct(t *testing.T) {
	...
	t.Run("simple", func(t *testing.T) {
		...
		if diff := cmp.Diff(args, []any{"name", 30, "first", "last"}); diff != "" {
	...
	t.Run("with transformer", func(t *testing.T) {
		...
		tr := func(name string, val any) any {
			if name == "age" {
				return 42
			}
			return val
		...
		if diff := cmp.Diff(args, []any{"name", 42, "first", "last"}); diff != "" {
			t.Error("args mismatch", diff)
		}
	})
	...
	t.Run("fallback with transformer", func(t *testing.T) {
		...
		if diff := cmp.Diff(args, []any{"name", 30, "first", "map_found"}); diff != "" {
			t.Error("args mismatch", diff)
		}
	})
...
func TestQueryxAllWrapped(t *testing.T) {
	...
	var (
		gocqlQueryPtr = reflect.TypeOf((*gocqlv2.Query)(nil))
		queryxPtr     = reflect.TypeOf((*Queryx)(nil))
	)

	for i := range gocqlQueryPtr.NumMethod() {
