...
//     In contrast, cqlx.qb.BatchBuilder, which is based on gocql.Query, prepares the whole batch statements as one ordinary query.
//
...
func (b *BatchBuilder) Query(session cqlx.Session) *cqlx.Queryx {
	return session.Query(b.ToCql())
}

// QueryContext returns query wrapped with context built on top of current BatchBuilder state.
func (b *BatchBuilder) QueryContext(ctx context.Context, session cqlx.Session) *cqlx.Queryx {
...
//
// USING TIMEOUT is a feature specific to ScyllaDB. Apache Cassandra rejects
// it with a SyntaxException (CASSANDRA-15193, Won't Fix). The method is kept
// for source-level compatibility with upstream gocqlx; see
// docs/api-deltas.md.
