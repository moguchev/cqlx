...
import (
	...
	"github.com/moguchev/cqlx/dbutil"
...
func TestRewriteTableTTL(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.rewrite_table (testtext text PRIMARY KEY)`); err != nil {
		t.Fatal("create table:", err)
	}

	tbl := table.New(table.Metadata{
		Name:    "cqlx_test.rewrite_table",
...
func TestRewriteTableClone(t *testing.T) {
	...
	if err := session.ExecStmt(`CREATE TABLE cqlx_test.rewrite_table_clone_src (testtext text PRIMARY KEY, testint int)`); err != nil {
		t.Fatal("create table:", err)
	}

	src := table.New(table.Metadata{
		Name:    "cqlx_test.rewrite_table_clone_src",
		Columns: []string{"testtext", "testint"},
		PartKey: []string{"testtext"},
	})

	if err := session.ExecStmt(`CREATE TABLE cqlx_test.rewrite_table_clone_dst (testtext text PRIMARY KEY, testfloat float)`); err != nil {
		t.Fatal("create table:", err)
	}

	dst := table.New(table.Metadata{
		Name:    "cqlx_test.rewrite_table_clone_dst",
	...
	transformer := func(m map[string]any) {
