...
func TestExample(t *testing.T) {
	...
	cqlxtest.BasicCreateAndPopulateKeyspace(t, session, "examples")
	createAndPopulateKeyspaceAllTypes(t, session)
	basicReadCassandraVersion(t, session)
...
// Song and PlaylistItem fixtures live in cqlxtest.
...
func createAndPopulateKeyspaceAllTypes(t *testing.T, session cqlx.Session) {
	t.Helper()
	...
	insertCheckTypes.BindStruct(CheckTypesStruct{
		...
		DoublE:     double,
		DuratioN:   gocqlv2.Duration{Months: 1, Days: 1, Nanoseconds: 86400},
		FloaT:      float,
		ID:         byteID,
		InT:        integer,
		IneT:       "127.0.0.1",
...
func basicReadCassandraVersion(t *testing.T, session cqlx.Session) {
	t.Helper()
	...
	t.Logf("Cassandra version is: %s", releaseVersion)
}
...
// consume.
func datatypesBlob(t *testing.T, session cqlx.Session) {
	t.Helper()
...
// adding a single line - embedding cqlx.UDT.
func datatypesUserDefinedType(t *testing.T, session cqlx.Session) {
	t.Helper()
...
func datatypesUserDefinedTypeWrapper(t *testing.T, session cqlx.Session) {
	t.Helper()
...
func datatypesJSON(t *testing.T, session cqlx.Session) {
	t.Helper()
...
func pagingFillTable(t *testing.T, insert *cqlx.Queryx) {
	t.Helper()
...
func pagingForwardPaging(t *testing.T, session cqlx.Session) {
	t.Helper()
...
func pagingEfficientFullTableScan(t *testing.T, session cqlx.Session) {
	t.Helper()
	...
	worker := func() error {
		...
		stmt, names, err := cqlx.CompileNamedQueryString(cql)
		if err != nil {
			return err
		}
...
func lwtLock(t *testing.T, session cqlx.Session) {
	t.Helper()
	...
	extend := func(lock Lock) bool {
		q := qb.Update("examples.lock").
			...
			TTLNamed("ttl").
			Query(session).
			SerialConsistency(gocqlv2.Serial).
			BindStruct(lock)
...
func unsetEmptyValues(t *testing.T, session cqlx.Session) {
	t.Helper()
	...
	insertQuery := insertOperation.Query(session).
		WithBindTransformer(cqlx.UnsetEmptyTransformer).
	...
	cqlx.DefaultBindTransformer = cqlx.UnsetEmptyTransformer
	defer func() {
		cqlx.DefaultBindTransformer = nil
	}()
